...
// 🎯 NUEVAS FUNCIONES: Generar y actualizar la barra del abecedario
function generarBarraAbecedario() {
    const barra = document.getElementById('barra-abecedario');
    barra.innerHTML = '';
    for (let i = 65; i <= 90; i++) {
        const span = document.createElement('span');
        span.textContent = String.fromCharCode(i);
        span.classList.add('letra-abecedario');
        span.setAttribute('data-letra', String.fromCharCode(i));
        barra.appendChild(span);
    }
}

function actualizarBarraAbecedario() {
    const letras = document.querySelectorAll('.letra-abecedario');
    letras.forEach(span => {
        const letra = span.getAttribute('data-letra');
        span.classList.remove('usada', 'actual');
        if (letra.charCodeAt(0) < letraActual) {
            span.classList.add('usada');
        } else if (letra.charCodeAt(0) === letraActual) {
...
function iniciarJuegoAlfabeto() {
    ...
    generarBarraAbecedario(); // 👈 Inicializar barra
    actualizarBarraAbecedario(); // 👈 Mostrar estado inicial
    ...
    function crearGlobo(letra) {
        ...
        globo.addEventListener('click', () => {
            if (letra === String.fromCharCode(letraActual)) {
                globo.remove();
                letraActual++;
                actualizarBarraAbecedario(); // 👈 Actualizar progreso
...
// Nivel de identificación (sin cambios)
...
function obtenerPosicionNoEncimada() {
    let nuevaPosicion;
    let intentos = 0;
    const distanciaMinima = 12;

    do {
        nuevaPosicion = Math.random() * 80 + 10;
        intentos++;
